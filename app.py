...
        color: #1f88e5;
        font-weight: 600;
    # 2x2 Layout for Metrics
col1, col2 = st.columns(2)
with col1:
    st.metric("Price", f"₹ {cmp:.2f}")
    st.metric("Market Cap", "3.3T")
with col2:
    st.metric("P/E Ratio", "15.2")
    st.metric("Dividend", "1.2%")
...
<div style="background-color:#111827; padding:15px; border-radius:8px; border-left:6px solid #2ea44f; margin-bottom:25px;">
    <b class="metric-profit">📈 Price Action Wave Reasoning: UPWARD BREAKOUT MOMENTUM</b>
    <p style="margin:5px 0 0 0; font-size:14px; color:#8b949e;">
        Sourcing volumes are currently outstripping localized floating ask buffers, creating an immediate momentum breakout trajectory across the timeline chart.
    </p>
</div>
